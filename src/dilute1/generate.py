...
def main() -> None:
    ...
    if args.teacher_model == DEFAULT_TEACHER and free_bytes < 5 * 1024**3:
        print(
            f"Only {free_bytes / 1024**3:.1f} GB is free; using {FALLBACK_TEACHER} "
            f"instead of downloading the {DEFAULT_TEACHER} checkpoint.",
            flush=True,
        )
        args.teacher_model = FALLBACK_TEACHER
